...
| 项目 | 内容 |
|---|---|
| 文档版本 | v0.9.1（相对 v0.9：语音默认改为 MiMo-V2.5，ST-01 不再手填 model） |
...
- 大语言模型（LLM）已具备稳定扮演考官、按评分标准点评的能力；云端 ASR/TTS（当前默认 Xiaomi MiMo-V2.5）可在普通 Android 设备上支撑按住说话的口语闭环。
...
| 指标 | 目标值 | 说明 |
|---|---|---|
| ASR 首字/首词延迟（边说边显字） | 系统引擎时代目标：首 partial ≤ 600ms | **当前 MiMo 路径不提供说话中 partial**；松手后出转写 |
| 说停到 ASR 最终结果 | 松手后上传 WAV → MiMo 返回转写 | 依赖网络与音频时长；Part 2 整段一次识别 |
...
| 机型差异 | 当前 ASR/TTS 走 MiMo 云端，**不依赖 GMS**；无网则无法识别/播报（见 04 风险清单） |

### 5.3 可用性与可靠性

- 断网/弱网：对话 / ASR / TTS 均需访问用户配置的 MiMo 端点；应明确告知「模型网络不可用」，并可结束会话、保留本地录音与已保存轮次（SY-03 本地）。
- 崩溃不丢数据：录音写盘 + 会话状态本地事务；App 被杀后重新进入可「恢复未完成会话」。
- 音频焦点：来电/其他 App 占用麦克风时自动暂停并恢复。

### 5.4 安全与隐私

- 个人 API Key：本机 **Android Keystore AES/GCM** 存储，绝不明文落盘或写入日志（不使用已停维护的 EncryptedSharedPreferences）。
- 录音与转写属**敏感个人数据**，分两路：会话 wav **仅本机**；识别片段会发往用户配置的 **MiMo ASR**。自有 2C2G 同步（SY）仍默认关闭，且服务器不存音频。
...
| 录音 | 本地优先（16 kHz PCM wav；10min 会话约十几 MB 量级），自有服务器不存音频；识别片段走 MiMo |
