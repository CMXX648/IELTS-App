...
| 项目 | 内容 |
|---|---|
| 文档版本 | v0.9.3（学习地图插画岛标 + 环形进度；语音默认 MiMo-V2.5） |
...
| 岛 | 状态 | 点击 |
|---|---|---|
| 🎙 发音热身岛 | 已完成徽章 + 麦克风岛标 + 环形 100% +「已完成」 | 家乡话题自由对话 |
| 💬 话题素材岛 | 今天练 + 对话气泡岛标 + 环形 45% +「点击开始练习 →」 | Part 1 |
| 🧠 逻辑表达岛 | 待解锁 + 书本岛标 + 环形 0% + 锁提示 | 不可点（完成后进 GR） |
| 📝 全真模考岛 | 终极挑战 + 奖杯岛标 + 环形 0% + 锁提示 | 不可点（全解锁后进完整模考） |

顶栏：`SPEAKING MASTER` + 问候名 + 连胜胶囊。信息卡宽 200dp。岛间为三点 + chevron 连接。长按问候区进 LatencySmoke。
