...
底部导航固定 4 个 Tab（手机形态）；大屏（sw≥600dp）同构但支持双栏布局（见 §7）。
...
**首启流程**（首次安装）：
`欢迎/引导页（3 屏：定位·隐私说明·用法） → 语言与目标设置（默认英语/总分7.5） → LLM 配置引导（ST-01） → 设备权限引导（麦克风） → 建议先做 S0 诊断`
...
#### 关键交互
1. **按住说话**：按下开始录音+识别（ASR partial 实时上屏）；松开 = 本轮说完。另提供**点击切换「聆听模式」**（适合免手场景）：自动端点检测（VAD）判断说完。
2. **边说边显字**：用户说话时其气泡内文字实时更新（partial 灰字 → 停顿稳定后变黑成 final）；误识可点「改」编辑该句文字（编辑后的文本作为入 LLM 的输入）。
...
- 系统/ML Kit ASR 不一定提供词级时间戳 → 以 **Turn/句段级** 对齐（VAD 切句点 + 录音本地时间戳估算）。云端 ASR 若提供词级时间戳则为增强项（04 文档）。
- EV 错误项的 tRange 映射到句段索引；无词级时间戳时高亮整个句段。

### 5.3 录音管理

- 会话录音 = 单一 m4a（AAC）文件 + 句段索引表（Room，本地）。
...
| 令牌 | 浅色值 | 深色值 | 用途 |
|---|---|---|---|
| Primary | `#4F46E5`（靛蓝） | `#818CF8` | 主操作、选中态 |
| Secondary | `#7C3AED`（紫罗兰） | `#A78BFA` | 渐变副色、语音主题 |
| PrimaryContainer | `#312E81`（深靛蓝） | `#3730A3` | 头部渐变、雷达图容器 |
| Background | `#F6F7FB` | `#0F1220` | 页面底 |
| Surface | `#FFFFFF` | `#161B2E` | 卡片 |
| OnSurface | `#1A1D29` | `#FFFFFF` | 主文本 |
| OnSurfaceVariant | `#5A6072` | `#B6BDD0` | 次级文本 |
| Success / Error / Warning / Info | `#10B981` `#EF4444` `#F59E0B` `#3B82F6` | 同左（亮度微调） | 判定/评分/提示 |
| 会话渐变 | `Linear(#4F46E5 → #7C3AED)` | 同左降亮度 | 头部/主按钮 |
...
| 底部导航 | 4 tab，图标 + 文字，选中态 Primary + 微弱上浮指示条 |
...
- 权限引导文案：说明「录音仅用于练习与分析，可离线使用，不默认上传」。

---

## 8. 数据所有权与可见性（贯穿全模块）

- 所有练习记录、评分、错题、收藏**默认本地生成、本地可见**；云同步为**显式开启**（SY，见 05）。
